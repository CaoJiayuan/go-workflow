...
type Procdef struct {
	Model
	Name    string `json:"name,omitempty"  gorm:"type:varchar(200);index"`
	Version int    `json:"version,omitempty"`
	// 流程定义json字符串
	Resource string `gorm:"type:json" json:"resource,omitempty"`
	// 用户id
	Userid   string `json:"userid,omitempty" gorm:"index"`
	Username string `json:"username,omitempty"`
	// 用户所在公司
	Company    string `json:"company,omitempty"`
	DeployTime string `json:"deployTime,omitempty" gorm:"type:timestamp"`
}
...
func MoveProcdefToHistoryByIDTx(ID int, tx *gorm.DB) error {
	err := tx.Exec("insert into procdef_histories select * from procdefs where id=?", ID).Error
	if err != nil {
		return err
	}
